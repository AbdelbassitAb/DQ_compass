...
(() => {
  ...
  s.addText("A plug-and-play Data Quality control layer for EUCs", {
    isTextBox: true, x: MX, y: 3.62, w: 11, h: 0.6, margin: 0, fontFace: BODY, fontSize: 19, color: "D8D4D0" });
  s.addText("Group 1   -   MBA-ESG x SG GSC Datathon 2026   -   5 min + demo", {
    isTextBox: true, x: MX, y: 4.5, w: 11, h: 0.4, margin: 0, fontFace: BODY, fontSize: 12, color: "8A8A93" });
  notes(s, "DQ Compass, Group 1. In 5 minutes: who we are and how we worked, the context, the solution in four building blocks - Catalogue, Engine, Reporting, Audit - and the perspectives. Then a 5-minute demo.");
})();
...
(() => {
  const s = p.addSlide(); s.background = { color: INK };
  s.addText("Over to the demo", { isTextBox: true, x: MX, y: 1.5, w: CW, h: 1.0, margin: 0,
    fontFace: HEAD, fontSize: 34, bold: true, color: WHITE });
  const steps = [
    "The Catalogue - create and validate a rule (the meta-catalogue rejects an invalid one).",
    "Run an execution - the engine applies the whole catalogue to the dataset.",
    "The Reporting - open an incident: the failing rows and the remediation action.",
    "An Evidence Pack - click Verify: the run replays identically.",
    "The Mapping - each control tied to a BCBS 239 requirement and to the run hash."];
  steps.forEach((t, i) => {
  ...
  footer(s, 10, true);
  notes(s, "Over to the demo, in five steps: create a rule in the catalogue, run an execution, open an incident in the reporting with the failing rows, open an Evidence Pack and click Verify, and finally the supervisory mapping.");
})();

p.writeFile({ fileName: "C:/Users/Basset/Downloads/dq_compass/dq_compass/docs/DQ_Compass_Presentation_5min_EN.pptx" })
